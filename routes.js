...
//  FURTHER STUDY 
  ...
  try {
    ...
    if (!name || name.trim() === "") {
      return res.render("search-customer.html", { customers: []}); 
    }

    const customers = await Customer.searchByName(name);

    return res.render("search-customer.html", { customers, searchedName });

  } catch (err) {
    console.error("Search error:", err.message); // Debug log
    return next(err);
  }
});
